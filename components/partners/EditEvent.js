...
export default function NewEvent() {
  ...
  const deletePhoto = () => {
    ...
    if (newPictures[photoIndex].file) {
      ...
      axios({
        method: 'delete',
        url: 'https://ws.tybyty.com/evento/eliminar/foto',
        data: {
  ...
  const save = () => {
    ...
    if (verify()) {
      ...
      formData.append("id_evento", route.params.id);
      ...
      axios({
        ...
        method: 'put',
        url: 'https://ws.tybyty.com/eventos',
        data: formData,
        headers: {
        ...
        if (response.status == 201) {
          ...
          Dialog.show({
            ...
            textBody: Language.get(language, 'Evento editado'),
            button: Language.get(language, 'Cerrar')
          });
        }
  ...
  const getEvent = () => {
    ...
    axios({
      method: 'get',
      url: 'https://ws.tybyty.com/evento/' + route.params.id,
      data: {},
      responseType: 'json',
      responseEncoding: 'utf8',
      headers: { 'Content-Type': 'application/json;charset=UTF-8' }
      ...
      if (response.status == 200) {
        ...
        let dt_evento = response.data.evento;

        setYear(getYear(dt_evento.fecha));

        setMonth(getMonth(dt_evento.fecha));

        setDay(getDate(dt_evento.fecha));

        setHour(getHour(dt_evento.fecha));

        setMinute(getMinute(dt_evento.fecha));

        setTitle(dt_evento.nombre);

        setDetail(dt_evento.descripcion);

        setProvince(dt_evento.provincia);

        setCategory(dt_evento.id_categoria);

        setLocation(dt_evento.lugar);

        setLocationData({

          lat: dt_evento.latitud,
          lon: dt_evento.longitud
        });

        setMen(dt_evento.hombres);

        setWomen(dt_evento.mujeres);

        setCouples(dt_evento.parejas);
  ...
  const options = () => {
    ...
    axios({
      method: 'get',
      url: 'https://ws.tybyty.com/eventos/categorias',
      data: {},
      responseType: 'json',
      responseEncoding: 'utf8',
      headers: { 'Content-Type': 'application/json;charset=UTF-8' }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAwareScrollView>
        ...
        <AlertNotificationRoot theme="dark">
          ...
          <Text style={{ ...styles.title, marginTop: 20 }}>
            {Language.get(language, "¿Quién puede venir a tu fiesta?")}
