...
export default function FvDetailEvent() {
    ...
    const renovate = () => {
        ...
        if (!renewal) {
            ...
            axios({
                method: 'post',
                url: 'https://ws.tybyty.com/evento/renovar',
                data: {
            ...
            axios({
                method: 'post',
                url: 'https://ws.tybyty.com/evento/parar/renovacion',
                data: {
                    "id_evento": route.params.id
                },
                responseType: 'json',
    ...
    const verifyEvent = () => {
        ...
        axios({
            method: 'get',
            url: 'https://ws.tybyty.com/evento/verificar/renueva/' + route.params.id,
            data: {},
            responseType: 'json',
            responseEncoding: 'utf8',
            headers: { 'Content-Type': 'application/json;charset=UTF-8' }
    ...
    const removeEvent = () => {
        ...
        axios({
            method: 'delete',
            url: 'https://ws.tybyty.com/evento',
            data: {
                "id": route.params.id
            },
            responseType: 'json',
            ...
            if (response.status == 200) {
                ...
                Dialog.show({
                    type: ALERT_TYPE.SUCCESS,
                    title: Language.get(language, 'Exito'),
                    textBody: Language.get(language, 'Evento eliminado'),
                    button: Language.get(language, 'Cerrar')
                });
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Modal isVisible={remove} onBackdropPress={() => setRemove(false)} backdropOpacity={0.70}>
                ...
                <View>
                    ...
                    <View style={modal.title}>
                        ...
                        <Text style={{ ...styles.text, color: "white", fontSize: 17, fontWeight: "bold" }}>
                            {Language.get(language, "Eliminar Evento")}
                        </Text>
                    ...
                    <KeyboardAwareScrollView style={modal.body}>
                        ...
                        <View style={modal.content}>
                            ...
                            <Text style={{ ...styles.text, color: "white", fontSize: 17 }}>{Language.get(language, "Estas seguro que deseas eliminar el evento?")}</Text>
            ...
            {(event && route.params) && <View style={{ flexDirection: "row", flexWrap: 'wrap', marginTop: 10, justifyContent: "center", alignItems: "center", backgroundColor: "#1F2122" }}>
                ...
                <Text style={{ ...styles.text, margin: 5 }} onPress={() => notReloadNavigate("EditEvent", { "id": event.id })}>
                    <MaterialIcons name="edit" size={15} color="#3374FF" /> {Language.get(language, "Editar")}
                </Text>

                <Text style={{ ...styles.text, margin: 5 }} onPress={() => verifyEvent()}>
                ...
                <Text style={{ ...styles.text, margin: 5 }} onPress={() => share("https://tybyty.com/socios/evento/compartir/" + event.id)}>
